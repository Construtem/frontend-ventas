...
import { clienteService, DBCotizacion, CotizacionCheckout } from "@/services/apiServices";
import NumberIcon from "@/components/NumberIcon";
import { checkoutCotizacion } from "@/services/apiServices"; // Asegúrate de tener esta función en tu proyecto
...
export default function CotizacionHeader() {
    ...
    /** seleccionar cotizacion existente y obtener detalles */
    const handleCotizacionChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        try {
            const cotizacionDetalle: CotizacionCheckout = await checkoutCotizacion(idSeleccionado);
            dispatch({ type: "SET_COTIZACION_SELECCIONADA", payload: cotizacionDetalle });
        } catch (err) {
            ...
            <div className="flex gap-[10px]">
                ...
                )}
                {
                    ...
                    <Button
                        ...
                        onClick={() => {
                            dispatch({ type: 'NUEVA_COTIZACION' });
                        }}
                    />
                }
            </div>
