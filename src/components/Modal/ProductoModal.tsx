...
export function ProductoModal({ isOpen, onClose }: ProductoModalProps) {
            ...
            .then(r => {
                ...
                const productosAjustados = r.productos.map(p => {
                    ...
                    productosAgregados.forEach(agregado => {
                        if (agregado.sku === p.sku && agregado.sucursalId === Number(sucursalId)) {
                ...
                // 🔧 Ya no limpiamos selQty ni selOrigen al abrir
                setSelOrigen(prev => ({ ...prev }))
                setSelQty(prev => ({ ...prev }))
            })
    ...
    const productosFiltrados = rows?.filter(p => {
    ...
    return (
        <>
            ...
            <Modal isOpen={isOpen} onClose={handleClose}>
                <ModalHeader title="Agregar productos" onClose={handleClose} />
                ...
                <ModalBody>
                    ...
                    {sucursalId && !state.clienteRut && (
                        <p className="text-gray-500 text-center">
                            Selecciona un cliente para cargar inventario.
                        </p>
                    )}
                                                ...
                                                <td className="px-2 py-1">
                                                    <div className="flex items-center gap-1 justify-center">
                                                        <button onClick={() => {
                                                            ...
                                                            setSelQty(q => ({...q, [p.sku]: Math.max(0, current - 1)}));
                                                        }}
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={e => {
                                                                ...
                                                                setSelQty(q => ({
                                                                    ...q,
                                                                    [p.sku]: val === "" ? "" : Math.min(Number(val), stock)
                                                                }));
                                                            }}
                                                        ...
                                                        <button onClick={() => {
                                                            setSelQty(q => ({
                                                                ...q,
                                                                [p.sku]: Math.min(stock, (Number(qty) || 0) + 1)
                                                            }));
                                                        }}
                                                                disabled={stock === 0 || Number(qty) >= stock}
                                                                className="px-[6px] border rounded bg-gray-100 hover:bg-gray-200 text-gray-700"
                                                        >+
                                                        </button>
                                                    </div>
                ...
                <ModalFooter>
                    <Button label="Cerrar" className="bg-gray-200" onClick={handleClose}/>
                </ModalFooter>
            </Modal>
        </>
    )
