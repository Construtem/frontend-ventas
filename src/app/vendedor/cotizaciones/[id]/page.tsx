...
/* ---------- Fallback para campos que la API aún no entrega ---------- */
const fallbackEntrega: Entrega = {
    tipo: 'Despacho',
...
const fallbackHistorial: HistorialEntrada[] = [
...
const EstadoBadge = ({ estado }: { estado: Cotizacion['estado'] }) => {
    const v = {
        ...
        Vencida  : { cls: 'bg-gray-100 text-gray-700', ico: <FaTimes /> },
    }[estado] ?? { cls: 'bg-gray-100 text-gray-700', ico: <FaClock /> };

    return (
        <span
            className={`inline-flex items-center gap-1 px-4 py-2 rounded-md text-sm font-semibold ${v.cls}`}
        >
      {v.ico}
            {estado}
    </span>
...
export default function CotizacionDetalle() {
    ...
    const [coti, setCoti] = useState<Cotizacion | null>(null);
        ...
        async function fetchData() {
            try {
                const res = await fetch(
                    `http://localhost:8080/api/cotizaciones/${id}`,
                );
                if (!res.ok) throw new Error('Network error');

                const api: Cotizacion = await res.json();
                setCoti({
                    ...api,
                    entrega: api.entrega ?? fallbackEntrega,
                    historial: api.historial.length ? api.historial : fallbackHistorial,
                });
            } catch {
                setCoti(null);
            } finally {
    ...
    if (loading)
        return <div className="ml-[180px] mt-[70px] p-8">Cargando…</div>;
    if (!coti)
        return (
            <div className="ml-[180px] mt-[70px] p-8 text-red-500">
                No se encontró la cotización.
            </div>
        );

    /* -------- cálculos -------- */
    const subtotal = coti.detalle_cotizacion.reduce(
    ...
    const totalFront =
        coti.total ?? subtotal - descuento + impuestos + coti.entrega.costo;
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="border border-gray-200 rounded-lg p-4 space-y-1">
                        ...
                        {coti.entrega.region && (
                            <p>
                                <span className="font-medium">Región/Comuna:</span>{' '}
                                {coti.entrega.region}, {coti.entrega.comuna}
                            </p>
                        )}
                        {coti.entrega.costo > 0 && (
                            <p>
                                <span className="font-medium">Costo despacho:</span>{' '}
                                ${coti.entrega.costo.toLocaleString('es-CL')}
                            </p>
                        )}
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                        ...
                        <tbody>
                        {coti.detalle_cotizacion.map(
                            (d: DetalleItem, i: number) => (
